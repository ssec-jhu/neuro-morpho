...
class BreaksAnalyzer:
    ...
    def analyze_breaks(self, pred_bin_img: np.ndarray, pred_img: np.ndarray) -> np.ndarray:
        ...
        while len(distances) >= 1: # Let's add components to the biggest one until there is only one left
            ...
            pred_bin_fixed_img = cv2.bitwise_or(pred_bin_fixed_img, line_mask) # Add the connection to the output binary imag
        ...
        # diff_target[(pred_bin_fixed_img == 255) & (target_image == 0)] = [0, 255, 0]  # Green: appears in fixed binary, missing in target
        # diff_target[(pred_bin_fixed_img == 0) & (target_image == 255)] = [255, 0, 0]  # Red: issing in fixed binary, appears in target
        ...
        # diff_bin[(pred_bin_fixed_img == 255) & (pred_bin_img == 0)] = [0, 255, 0]  # Green: appears in fixed binary, missing in orig binary
        # diff_bin[(pred_bin_fixed_img == 0) & (pred_bin_img == 255)] = [255, 0, 0]  # Red: issing in fixed binary, appears in orig binary
